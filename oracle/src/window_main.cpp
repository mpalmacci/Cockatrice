...
const QString WindowMain::defaultSetsUrl = QString(
...
WindowMain::WindowMain(QWidget *parent) :
	...
	aLoadSetsFile = new QAction(tr("Load sets information from &file..."),
			this);
	...
	connect(aDownloadSetsFile, SIGNAL(triggered()), this,
			SLOT(actDownloadSetsFile()));
...
void WindowMain::actLoadSetsFile() {
	...
	if (importer->readSetsFromFile(fileName))
		...
		QMessageBox::critical(this, tr("Error"),
				tr("This file does not contain any sets data."));
}

void WindowMain::actDownloadSetsFile() {
	QString url = QInputDialog::getText(this, tr("Load sets from URL"),
			tr("Please enter the URL of the sets file:"), QLineEdit::Normal,
			defaultSetsUrl);
	if (!url.isEmpty())
...
void WindowMain::updateTotalProgress(int cardsImported, int setIndex,
		const QString &nextSetName) {
	if (setIndex != 0)
		messageLog->append(
				QString("%1: %2 cards imported").arg(nextSetLabel2->text()).arg(
						cardsImported));
	totalProgressBar->setValue(setIndex);
	if (nextSetName.isEmpty()) {
		QMessageBox::information(this, tr("Oracle importer"),
				tr("Import finished: %1 cards.").arg(
						importer->getCardList().size()));
		bool ok = false;
		const QString dataDir = QDesktopServices::storageLocation(
		...
		do {
			...
			if (savePath.isEmpty())
				fileName = QFileDialog::getSaveFileName(this,
						tr("Save card database"), dataDir + "/cards.xml",
...
void WindowMain::actStart() {
	...
	if (!setsCount) {
		QMessageBox::critical(this, tr("Error"),
				tr("No sets to download selected."));
		return;
	}
	for (int i = 0; i < checkBoxList.size(); ++i)
